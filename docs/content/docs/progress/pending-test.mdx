...
- CI 工作流：新增 GitHub Actions `CI` workflow，推送 `main` / `cicd` 或发起对应 PR 时会检查 Web、文档站和 Canvas Agent；需在 GitHub Actions 页面确认触发和结果。
- GitHub Pages：前端静态站点发布 workflow 已支持推送 `cicd` 分支后自动构建 `web/dist` 并发布；需在仓库 Pages 设置中选择 GitHub Actions 后验证访问到前端页面而不是 README。
